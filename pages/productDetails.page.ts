...
import { Cart } from '@pages/common-components/cart';
...
export class ProductDetailsPage extends BasePage {
    ...
    constructor(page: Page) {
        ...
        this.productCode = this.page.locator('span:has-text("Product Code") + span');
        this.productBrand = this.page.locator('span:has-text("Brand") + a');
        this.productAvailability = this.page.locator('span:has-text("Availability") + span');
        this.productViews = this.page.locator('span:has-text("Viewed") + span');
        this.addToCartButton = this.page.getByRole('button', { name: "Add to Cart" });
        this.buyNowButton = this.page.getByRole('button', { name: "Buy Now" });
        this.toastMessageBody = this.page.locator('.toast-body p');
        this.toastCloseButton = this.page.getByRole('button', { name: 'Close' });
    }

    async waitForProductTitles(productName: string) {
        await this.page.locator(`h1:has-text("${productName}")`).waitFor({ state: 'visible' });
    }
